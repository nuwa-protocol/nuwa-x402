// Lightweight logger used inside the @nuwa-ai/x402 package
// Mirrors the minimal logger used in the example app but without app-specific deps
...
const rootLogger = new Logger(["nuwa-ai", "x402"], resolveLogLevel());

export function createLogger(scope: string | string[]) {
  const segments = Array.isArray(scope) ? scope : scope.split(":");
  return segments.reduce((current, segment) => current.child(segment), rootLogger);
}
